...
BER_theoretical = zeros(size(Ptx_dbm_range));
...
for ptx_idx = 1:length(Ptx_dbm_range)
    ...
    BER_theoretical(ptx_idx) = (2/log2(M))*(1-1/sqrt(M))*erfc(sqrt(1.5*SNR/(M-1)));
    SER_theoretical(ptx_idx) = 1-(1-BER_theoretical(ptx_idx))^log2(M);
...
figure;
semilogy(Ptx_dbm_range, SER_simulated, 'bo-', 'DisplayName', 'Simulated 64-QAM');
hold on;
semilogy(Ptx_dbm_range, SER_theoretical, 'r--', 'DisplayName', 'Theoretical 64-QAM');
