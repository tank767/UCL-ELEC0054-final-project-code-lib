...
SER_simulated = zeros(size(lambda_range));
SER_theoretical = zeros(size(lambda_range));
BER_theoretical = zeros(size(lambda_range));

for lambda_idx = 1:length(lambda_range)
    current_lambda = lambda_range(lambda_idx);
    current_beta2 = beta2_range(lambda_idx);
...
figure;
semilogy(lambda_range, SER_simulated, 'bo-', 'DisplayName', 'Simulated 64-QAM');
hold on;
semilogy(lambda_range, SER_theoretical, 'rx-', 'DisplayName', 'Theoretical 64-QAM');
xlabel('wavelength');
