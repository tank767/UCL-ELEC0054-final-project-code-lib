...
M_range = [4 8 16 32 64 128 256];                    % MQAM
...
for M_idx = 1:length(M_range)
    ...
    modulated_signal = qammod(bits, M_range(M_idx));

    % Pass the modulated signal through an AWGN channel
    received_signal = awgn(modulated_signal, SNR_dB, "measured");
    
    % Demodulate the received signal
    received_bits = qamdemod(received_signal, M_range(M_idx));
    
    % Calculate the simulated BER
    SER_simulated(M_idx) = sum(bits ~= received_bits)/N;
    
    % Calculate the theoretical BER for QPSK in AWGN
    BER_theoretical(M_idx) = (2/log2(M_range(M_idx)))*(1-1/sqrt(M_range(M_idx)))*erfc(sqrt(1.5*SNR/(M_range(M_idx)-1)));
...
figure;
semilogy(M_range, SER_simulated, 'bo-', 'DisplayName', 'Simulated QAM');
hold on;
semilogy(M_range, SER_theoretical, 'rx-', 'DisplayName', 'Theoretical QAM');
xlabel('M number');
ylabel('Symbol Error Rate (SER)');
title('QAM Modulation over AWGN Channel');
grid on;
ylim([1e-8 1]);
legend('show');
